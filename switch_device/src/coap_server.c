...
// modified
static void on_switch_request(uint8_t command)
{
	switch (command) {
	case THREAD_COAP_UTILS_SWITCH_CMD_ON:
		turn_on_switch();
		break;
	case THREAD_COAP_UTILS_LIGHT_CMD_OFF:
		turn_off_switch();
...
int main(void)
{
	...
	ret = ot_coap_init(&deactivate_provisionig, &on_light_request, &on_switch_request);
	if (ret) {
